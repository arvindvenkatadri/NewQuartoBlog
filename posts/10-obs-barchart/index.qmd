...
This post explores the use of [Observable](https://observablehq.com/)
(OJS) for data visualization, particularly using the `plot` library. We
will use the `penguins` dataset from the `palmerpenguins` package.
...
Let's try the same plot with facetting, AND with change in colour
palette:
